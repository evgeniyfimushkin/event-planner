...
Каждый сервис имеет свою изолированную SQL базу данных PostgreSQL.

![](../reports/images/Pasted%20image%2020250312205448.png)
![](../reports/images/image_2025-03-12_18-19-38.png)
...
Визуализация
Grafana - мощный инструмент визуализации мониторинга систем. В качестве источников данных использует Prometheus и Loki.
...
По итогу в нашей инфраструктуре два стэка контейнеров: стэк контейнеров приложения и стэк контейнеров мониторинга.
...
Для управление инфраструкторой используется подход инфраструкты как коды(Infrastructure as Code, IaC), в частности используется инструмент Terraform, где необходимые ресурсы в декларативном формате на языке HCL, а затем инструмент автоматически создаёт, изменяет или удаляет ресурсы из облака, приводя инфраструктуру в соответствие с описание. Также этот подход позволяет упаковать инфраструктуру в Git для отслеживания версий.
